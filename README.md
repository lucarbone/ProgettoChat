...
Durante la creazione del server, nella creazione del Server Socket abbiamo notato che se si facevano partire due server con la stessa porta, entrambi i server funzionavano, però mentre uno faceva tutte le operazioni normalmente, l’altro rimaneva aperto e non faceva nulla, per risolvere questo problema, prima di far partire il Server Socket abbiamo messo un controllo nel quale, verifica che la porta scelta dall’utente non sia già in utilizzo, nel caso sia già utilizzata il server dice all’utente che la porta è già in uso e di sceglierne un altra.
