...
Durante la scrittura della parte server, nella parte della lista dei client connessi, abbiamo avuto un paio di problemi, per esempio la lista del client non andava correttamente e non mostrava il numero di utenti connessi corretto, oppure non rimuoveva il cliente quando si disconnetteva;
Per sistemare questi problemi abbiamo corretto alcune parti del ciclo che crea le caselle dei client, abbiamo implementato un sistema in cui, ogni volta che il pannello dei clienti connessi deve essere aggiornato, tutti i client vengono ridisegnati da zero. Infine il pannello al posto di doversi aggiornare ogni tot secondi, si aggiorna quando un cliente si connette, inserisce il nome o si disconnette.
